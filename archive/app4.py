...
# 4. AGGRESSIVELY REFINED SYSTEM PROMPT for Llama 3
...
def chat_with_bot(user_message):
    ...
    try:
        ...
        # 2. Emergency Contact Handling (with LLM translation)
        ...
        if is_emergency_query and st.session_state.emergency_data is not None:
            ...
            if city:
                ...
                if emergency_info_en:
                    # Use LLM to translate the found contact info
                    translation_prompt = f"The user asked for emergency contact. The English data found is: '{emergency_info_en}'. The user's original query was: '{user_message}'. You MUST translate the English data and MUST respond in the language of the user's original query, keeping the numbers/cities clear."
...
with st.sidebar:
    ...
    st.markdown(f"**Selected LLM:** `{LLM_MODEL}` (Llama 3 70B)")
