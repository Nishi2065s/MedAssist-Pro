    ...
    try:
        ...
        if is_emergency_query and st.session_state.emergency_data is not None:
            ...
            if city:
                ...
                if emergency_info:
                    # Return emergency info directly without going through LLM
                    st.session_state.conversation_memory.append({
                        "user": user_message,
                        "assistant": emergency_info
                    })
                    return emergency_info
                else:
                    available_cities = ", ".join(st.session_state.emergency_data.iloc[:, 0].head(10).tolist())
                    emergency_context = f"\n\n[DATABASE]: City '{city}' not found. Tell user these cities are available: {available_cities}"
            else:
                available_cities = ", ".join(st.session_state.emergency_data.iloc[:, 0].head(10).tolist())
                emergency_context = f"\n\n[INSTRUCTION]: Ask user to specify city name from: {available_cities}"
